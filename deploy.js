...
async function main() {
    ...
    fs.writeFileSync("translations.node.js", exported, "utf-8");
    console.log(green("✔ translations.node.js created."));

    // 2-4. Build minified assets
    await buildMinifiedAssets(original, originalScript);

    // 5. Load translations
    delete require.cache[require.resolve("./translations.node.js")];
    ...
    for (const htmlFile of htmlFiles) {
        ...
        for (const lang of Object.keys(translations)) {
            ...
            // Replace known special selectors
            ...
            for (const [selector, key] of Object.entries(specialMap)) {
                ...
                if (el && langDict[key]) {
                    if (el.tagName === "TITLE") {
                        ...
                        el.setAttribute("content", langDict[key]);
                    }
                }
            }
            ...
            doc.querySelectorAll("body *:not([data-i18n])").forEach((el) => {
                ...
                const autoKey = Object.keys(langDict).find((key) => {
                    return (
                        (translations.hu && translations.hu[key] === text) ||
                        langDict[key] === text
                    );
                });
    ...
    // 13. Log unused translation keys
    ...
    Object.entries(translations).forEach(([lang, langData]) => {
        Object.keys(langData)
            .filter((key) => !usedKeys.has(key) && !specialKeys.includes(key))
            .forEach((key) => {
                unusedTable.push({ key, lang, file: "translations.js" });
            });
    });

    if (unusedTable.length > 0) {
        const col1 = "key";
        const col2 = "language";
        const col3 = "file";

        const col1Len = Math.max(...unusedTable.map((r) => r.key.length), col1.length);
        const col2Len = Math.max(...unusedTable.map((r) => r.lang.length), col2.length);
        const col3Len = Math.max(...unusedTable.map((r) => r.file.length), col3.length);
    ...
    // 14. Copy .htaccess file from template
    ...
    // 15. Copy contents of favicons/ directly into dist/
...
async function buildMinifiedAssets(original, originalScript) {
    const minifiedTranslationsResult = await minify(original, {
        compress: true,
        mangle: false,
        format: {
            comments: false
    ...
    fs.writeFileSync("translations.min.js", minifiedTranslationsResult.code || "", "utf-8");
    console.log(green("✔ translations.min.js created."));

    const minifiedScriptResult = await minify(originalScript, {
        compress: true,
        mangle: false,
        format: {
            comments: false
    ...
    fs.writeFileSync("script.min.js", minifiedScriptResult.code || "", "utf-8");
    console.log(green("✔ script.min.js created."));

    const cssOriginal = fs.readFileSync("style.css", "utf-8");
    const cssMinifiedResult = new CleanCSS({
        level: 2
    }).minify(cssOriginal);

    if (cssMinifiedResult.errors && cssMinifiedResult.errors.length > 0) {
        console.error(red("❌ CSS minification failed:"));
        cssMinifiedResult.errors.forEach((err) => console.error(red(err)));
        process.exit(1);
    }

    fs.writeFileSync("style.min.css", cssMinifiedResult.styles || "", "utf-8");
...
function updateLanguageSelect(doc, currentLang) {
    ...
    options.forEach((opt) => {
        const match = opt.value.match(/^\/([a-z]{2})\//);
        const langFromValue = match ? match[1] : null;
...
main().catch((error) => {
    console.error(red("❌ Deploy failed."));
    console.error(error);
    process.exit(1);
});